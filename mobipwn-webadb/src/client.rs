...
use super::protocol::{
    AdbError, Command, ConnectionState, Message, Stream, ADB_CNXN_MAXDATA, ADB_VERSION,
    ADB_VERSION_SKIP_CHECKSUM, AuthType, CNXN_HOST_BANNER,
};
...
impl AdbClient {
    ...
    pub async fn shell_with_timeout(&mut self, command: &str, timeout_ms: u32) -> Result<String, AdbError> {
        const MARKER: &str = "__MOBIPWN_SHELL_DONE__";
        let wrapped = if command.contains(MARKER) {
            command.to_string()
        } else {
            format!("({}); echo {MARKER}", command)
        };
        let dest = format!("shell:{wrapped}");
        ...
        loop {
            ...
            if output.len() >= marker_bytes.len()
                && output.windows(marker_bytes.len()).any(|w| w == marker_bytes)
            {
                break;
            }
    ...
    pub async fn connect(&mut self) -> Result<(), AdbError> {
        ...
        // ADB 0x01000001+ allows peers to omit payload checksums in the header; strict RX verify fails.
        self.transport
            .set_relaxed_rx_checksum(ADB_VERSION >= ADB_VERSION_SKIP_CHECKSUM);

        // Send CNXN: banner must follow ADB grammar (`host::…;\0`), not arbitrary text after `::`.
        ...
        self.transport.send_message(&message, CNXN_HOST_BANNER).await?;

        log::debug!(
            "[ADB] CNXN sent (version={:#x}, maxdata={}), waiting for device…",
            ADB_VERSION,
            ADB_CNXN_MAXDATA
        );
    ...
    pub async fn shell(&mut self, command: &str) -> Result<String, AdbError> {
        self.shell_with_timeout(command, 120_000).await
    }
    ...
    pub async fn push_file(&mut self, data: &[u8], remote_path: &str) -> Result<(), AdbError> {
        ...
        // Send SEND command with path and mode (0755 for executables under /data/local/tmp)
        let path_and_mode = format!("{},0755", remote_path);
        ...
        // Samsung (and some other builds) finish the write but never send sync DONE on this
        // stream — waiting in read_stream() blocks forever on USB recv. Close and verify size.
        let _ = self.close_stream(local_id).await;
        self.verify_remote_file_size(remote_path, data.len()).await
    }

    async fn remote_file_size_shell(&mut self, remote_path: &str) -> Result<usize, AdbError> {
        ...
        out.trim()
            ...
            .ok_or_else(|| AdbError::IoError(format!("Could not stat {remote_path}")))
    }
    ...
    async fn verify_remote_file_size(
        ...
        let size = self.remote_file_size_shell(remote_path).await?;
        if size == expected {
            Ok(())
        } else {
            Err(AdbError::IoError(format!(
                "Push size mismatch for {remote_path}: expected {expected} bytes, got {size}"
            )))
        }
    }
