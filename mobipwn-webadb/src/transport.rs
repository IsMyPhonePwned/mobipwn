...
use web_sys::{
    UsbConfiguration, UsbDevice, UsbDeviceFilter, UsbDeviceRequestOptions, UsbDirection,
...
pub struct WebUsbTransport {
    ...
    endpoint_out: u8,
    interface_number: u8,
    /// When true, accept RX payloads even if `data_crc32` does not match (ADB ≥ 0x01000001 peers).
    relaxed_rx_checksum: Cell<bool>,
}

impl WebUsbTransport {
    /// Enable when our CNXN uses ADB version `0x01000001`+ — peers may omit valid `data_crc32` on RX.
    pub fn set_relaxed_rx_checksum(&self, relaxed: bool) {
        self.relaxed_rx_checksum.set(relaxed);
    }
    ...
    pub async fn request_device() -> Result<Self, AdbError> {
        ...
        // web-sys 0.3.9x: `new` takes `&[UsbDeviceFilter]` (older crates used `&js_sys::Array`).
        let options = UsbDeviceRequestOptions::new(std::slice::from_ref(&filter));
    ...
    pub async fn open(device: UsbDevice) -> Result<Self, AdbError> {
        ...
        // Some Android builds leave `configuration === null` until an explicit
        // `selectConfiguration()`; without it the ADB interface is invisible and the phone never
        // shows the USB debugging / RSA prompt.
        if device.configuration().is_none() {
            let configs = device.configurations();
            if configs.length() > 0 {
                let cfg: UsbConfiguration = configs
                    .get(0)
                    .dyn_into()
                    .map_err(|_| AdbError::UsbError("Bad USB configuration list".to_string()))?;
                let value = cfg.configuration_value();
                let sel = device.select_configuration(value);
                JsFuture::from(sel).await.map_err(|e| {
                    AdbError::UsbError(format!("select_configuration failed: {:?}", e))
                })?;
            }
        }
        ...
        Ok(Self {
            ...
            endpoint_out,
            interface_number,
            relaxed_rx_checksum: Cell::new(false),
        })
    }
    ...
    pub async fn recv_message(&self) -> Result<(Message, Vec<u8>), AdbError> {
        ...
        let data = if message.data_length > 0 {
            ...
            // ADB ≥ 0x01000001: peers often send a placeholder `data_crc32`; payload length and
            // bytes are still valid. Do not log here — it fires on many normal packets and reads
            // like a transport error to users.
            if !message.verify_data(&data) && !self.relaxed_rx_checksum.get() {
