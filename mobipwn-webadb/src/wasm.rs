...
fn wasm_ensure_adb_keypair() -> Result<AdbKeyPair, JsValue> {
...
/// JavaScript `Number` safe integer range — large `u64` / `i64` in JSON are emitted as strings.
#[cfg(feature = "bugreport-analysis")]
fn sanitize_json_integers_for_js_bugreport(v: serde_json::Value) -> serde_json::Value {
    ...
    match v {
        Value::Number(n) => {
            if let Some(i) = n.as_i64() {
                if (JS_MIN_SAFE_INT..=JS_MAX_SAFE_INT).contains(&i) {
                    return Value::Number(n);
                }
                return Value::String(n.to_string());
            }
            if let Some(u) = n.as_u64() {
                if u <= JS_MAX_SAFE_INT as u64 {
                    return Value::Number(n);
                }
                return Value::String(n.to_string());
            }
        ...
        Value::Array(items) => Value::Array(
            items
                .into_iter()
                .map(sanitize_json_integers_for_js_bugreport)
                .collect(),
        ),
        Value::Object(map) => Value::Object(
            map.into_iter()
                .map(|(k, v)| (k, sanitize_json_integers_for_js_bugreport(v)))
                .collect(),
        ),
        other => other,
    }
...
/// RFC-4180-style CSV of timeline rows (same logical fields as `jsonl`, column order stable).
#[cfg(feature = "bugreport-analysis")]
fn bugreport_timesketch_csv_from_events(events: &[serde_json::Value]) -> String {
    use serde_json::Value;
    ...
    fn value_to_csv_cell(v: &Value) -> String {
        match v {
            ...
            Value::String(s) => csv_escape_cell(s),
            Value::Bool(b) => {
                if *b {
                    "true".to_string()
                } else {
            ...
            Value::Array(_) | Value::Object(_) => {
                csv_escape_cell(&serde_json::to_string(v).unwrap_or_default())
            }
        }
    }
    ...
    for (i, c) in columns.iter().enumerate() {
        ...
        out.push_str(&csv_escape_cell(c));
    }
    out.push('\n');
    ...
    for ev in events {
        ...
        for (i, col) in columns.iter().enumerate() {
            ...
            let cell = obj.get(col).map(value_to_csv_cell).unwrap_or_default();
            out.push_str(&cell);
        }
        out.push('\n');
    }
...
impl Adb {
    ...
    pub async fn connect(&mut self) -> Result<JsValue, JsValue> {
        ...
        let keypair = wasm_ensure_adb_keypair()?;
        ...
        self.finish_connect(transport, keypair).await
    }
    ...
    /// the device here.
    #[wasm_bindgen(js_name = connectWithUsbDevice)]
    pub async fn connect_with_usb_device(&mut self, usb_device: JsValue) -> Result<JsValue, JsValue> {
        log::info!("[WASM] connectWithUsbDevice() called");
        let keypair = wasm_ensure_adb_keypair()?;

        let device: UsbDevice = usb_device.dyn_into().map_err(|_| {
            JsValue::from_str(
                "Expected a WebUSB USBDevice (from navigator.usb.requestDevice).",
            )
        })?;

        let transport = WebUsbTransport::open(device)
            .await
            .map_err(|e| {
                log::error!("[WASM] Failed to open USB device: {}", e);
                JsValue::from_str(&e.to_string())
            })?;
    ...
    async fn finish_connect(
        ...
        log::info!(
            "[WASM] Device: vendor_id={}, product_id={}, serial={:?}",
    ...
    pub async fn shell(&mut self, command: String) -> Result<String, JsValue> {
        ...
        log::info!("[WASM] Executing: {}", command);
        let result = client
            .shell(&command)
            .await
            .map_err(|e| {
    ...
    pub async fn shell_with_timeout(&mut self, command: String, timeout_ms: u32) -> Result<String, JsValue> {
        log::info!(
            "[WASM] shell_with_timeout() called (timeout={}ms): {}",
            timeout_ms,
            command
        );
        let client = self.client.as_mut()
        ...
        log::info!("[WASM] Executing: {}", command);
        let result = client.shell_with_timeout(&command, timeout_ms)
            .await
            .map_err(|e| {
                log::error!("[WASM] shell_with_timeout() failed: {}", e);
                JsValue::from_str(&e.to_string())
            })?;
        log::info!(
            "[WASM] shell_with_timeout() completed, result length: {} chars",
            result.len()
        );
        Ok(result)
    }
    ...
    pub async fn analyze_bugreport(&self, data: Vec<u8>, rules: String) -> Result<JsValue, JsValue> {
        ...
        use bugreport_extractor_library::enrich_parser_results;
        use bugreport_extractor_library::parsers::{
            Parser as DataParser, ParserType, AdbParser, AuthenticationParser, BatteryParser,
            BluetoothParser, CrashParser, DevicePolicyParser, HeaderParser, LogcatParser,
            MemoryParser, NetworkParser, PackageParser, PowerParser, PrivacyParser, ProcessParser,
            SamsungSfsLogsParser, UsbParser, VpnParser,
        };
        ...
        log::info!("  🧠 [ANALYZE] Creating MemoryParser...");
        if let Ok(p) = MemoryParser::new() {
            parsers_to_run.push((ParserType::Memory, Box::new(p)));
            log::info!("  ✅ [ANALYZE] MemoryParser created");
        } else {
            log::warn!("  ⚠️ [ANALYZE] Failed to create MemoryParser");
        }
        log::info!("  📜 [ANALYZE] Creating DevicePolicyParser...");
        if let Ok(p) = DevicePolicyParser::new() {
            parsers_to_run.push((ParserType::DevicePolicy, Box::new(p)));
            log::info!("  ✅ [ANALYZE] DevicePolicyParser created");
        } else {
            log::warn!("  ⚠️ [ANALYZE] Failed to create DevicePolicyParser");
        }
        log::info!("  🔧 [ANALYZE] Creating AdbParser...");
        if let Ok(p) = AdbParser::new() {
            parsers_to_run.push((ParserType::Adb, Box::new(p)));
            log::info!("  ✅ [ANALYZE] AdbParser created");
        } else {
            log::warn!("  ⚠️ [ANALYZE] Failed to create AdbParser");
        }
        log::info!("  🔑 [ANALYZE] Creating AuthenticationParser...");
        if let Ok(p) = AuthenticationParser::new() {
            parsers_to_run.push((ParserType::Authentication, Box::new(p)));
            log::info!("  ✅ [ANALYZE] AuthenticationParser created");
        } else {
            log::warn!("  ⚠️ [ANALYZE] Failed to create AuthenticationParser");
        }
        log::info!("  🔒 [ANALYZE] Creating VpnParser...");
        if let Ok(p) = VpnParser::new() {
            parsers_to_run.push((ParserType::Vpn, Box::new(p)));
            log::info!("  ✅ [ANALYZE] VpnParser created");
        } else {
            log::warn!("  ⚠️ [ANALYZE] Failed to create VpnParser");
        }
        log::info!("  🛡️ [ANALYZE] Creating PrivacyParser...");
        if let Ok(p) = PrivacyParser::new() {
            parsers_to_run.push((ParserType::Privacy, Box::new(p)));
            log::info!("  ✅ [ANALYZE] PrivacyParser created");
        } else {
            log::warn!("  ⚠️ [ANALYZE] Failed to create PrivacyParser");
        }
        if let Ok(p) = LogcatParser::new() {
            parsers_to_run.push((ParserType::Logcat, Box::new(p)));
        }
        if let Ok(p) = SamsungSfsLogsParser::new() {
            parsers_to_run.push((ParserType::SamsungSfsLogs, Box::new(p)));
        }
        ...
        if zip_utils::is_zip_file(data_u8) {
            if let Ok(sfs) = SamsungSfsLogsParser::parse_from_zip_bytes(data_u8) {
                let count = sfs.get("count").and_then(|c| c.as_u64()).unwrap_or(0);
                if count > 0 {
                    if let Some((_, slot, _)) = results
                        .iter_mut()
                        .find(|(pt, _, _)| *pt == ParserType::SamsungSfsLogs)
                    {
                        *slot = Ok(sfs);
                    } else {
                        results.push((
        ...
        let timeline_exp = export_timeline(&results);

        fn parser_ui_key(pt: ParserType) -> &'static str {
            match pt {
                ...
                ParserType::DevicePolicy => "device_policy",
                ParserType::Adb => "adb",
                ParserType::Authentication => "authentication",
                ParserType::Vpn => "vpn",
                ParserType::Privacy => "privacy",
                ParserType::Logcat => "logcat",
                ParserType::SamsungSfsLogs => "samsung_sfs_logs",
            }
        }

        let parser_outputs: serde_json::Value = {
            let mut m = serde_json::Map::new();
            for (pt, res, dur) in &results {
                let key = parser_ui_key(*pt);
        ...
        let timeline_csv = bugreport_timesketch_csv_from_events(&timeline_exp.events);
        let timesketch_block = serde_json::json!({
            "analyser": "timesketch",
            "count": timeline_exp.count,
            "skipped_epoch_timeline_events": timeline_exp.skipped_epoch_timeline_events,
            "events": timeline_exp.events,
            "jsonl": timeline_exp.jsonl,
            "csv": timeline_csv,
            "saf_comparison": "Plaso/Timesketch-style rows from bugreport parsers (message, datetime, timestamp μs, bugreport_parser, data_type, event_time_binding, time_is_approximate). per_record = time from the parsed row. snapshot_only = datetime is only the dumpstate capture instant from the bugreport header (or raw unparseable header string); used for e.g. ps/top, memory snapshots, socket listings, USB/Bluetooth lists, battery app summaries, power reset reasons, ANR traces without file time. system_fallback = no usable header timestamp; datetime is wall clock at export. Rows with timestamp before 1971-01-01 UTC (or datetime in 1970) are omitted; see skipped_epoch_timeline_events. Sigma rule hits are merged in the UI only (not in CSV/JSONL)."
        });
        let mut analysis_root = serde_json::to_value(&summary).map_err(|e| {
            log::error!("❌ [ANALYZE] Failed to serialize summary: {}", e);
            JsValue::from_str(&format!("serialize summary: {}", e))
        })?;
        if let Some(obj) = analysis_root.as_object_mut() {
            obj.insert(
                "analysers".into(),
                serde_json::json!({ "timesketch": timesketch_block }),
            );
            obj.insert("parser_outputs".into(), parser_outputs);
        }
        ...
        log::info!(
            "🗓️ [ANALYZE] Timesketch-style timeline: {} events (skipped {} bogus 1970-era rows)",
            timeline_exp.count,
            timeline_exp.skipped_epoch_timeline_events
        );

        // Nested `serde_json::Value` does not round-trip through `serde_wasm_bindgen::to_value` as
        // plain arrays/objects in JS (Vec fields become opaque Maps / wrong shapes). Encode as
        // JSON text; the page uses `JSON.parse` for a normal object tree.
        let analysis_for_js = sanitize_json_integers_for_js_bugreport(analysis_root);
        let analysis_str = serde_json::to_string(&analysis_for_js).map_err(|e| {
            log::error!("❌ [ANALYZE] Failed to serialize analysis JSON: {}", e);
            JsValue::from_str(&format!("serialize analysis JSON: {}", e))
        })?;
        Ok(JsValue::from_str(&analysis_str))
    }
