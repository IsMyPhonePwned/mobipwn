...
/// ADB protocol version we advertise in CNXN (`0x01000001` = Android 9+; enables 1 MiB payloads).
pub const ADB_VERSION: u32 = 0x01000001;

/// From this version onward, peers may send placeholder values in `data_crc32` (checksum not validated).
/// Strict RX verification rejects those packets — see `WebUsbTransport::set_relaxed_rx_checksum`.
pub const ADB_VERSION_SKIP_CHECKSUM: u32 = 0x01000001;

/// `arg1` (maxdata) for our initial CNXN — must match the version above (1 MiB for `0x01000001`).
pub const ADB_CNXN_MAXDATA: u32 = 1024 * 1024;

/// Host banner for the first CNXN packet: `DeviceIdentifier :: ParameterList \0` (see ADB docs).
/// A string like `host::rust-webadb:serial` is **not** valid parameter syntax and can make adbd
/// ignore the packet so the host hangs waiting for AUTH/CNXN.
pub const CNXN_HOST_BANNER: &[u8] = b"host::features=shell_v2,cmd,stat_v2,ls_v2;\0";
...
mod tests {
    ...
    fn test_message_serialization() {
        ...
        let message = Message::new(Command::Cnxn, ADB_VERSION, ADB_CNXN_MAXDATA, data);
        ...
        assert_eq!(deserialized.arg0, ADB_VERSION);
        assert_eq!(deserialized.arg1, ADB_CNXN_MAXDATA);
