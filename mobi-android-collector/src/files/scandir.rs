...
fn hash_file(path: &str, max_bytes: u64) -> String {
    let Ok(mut file) = fs::File::open(path) else {
        return String::new();
    };
    let mut hasher = Sha256::new();
    let mut limited = file.by_ref().take(max_bytes);
    if io::copy(&mut limited, &mut hasher).is_err() {
        return String::new();
    }
...
fn create_entry(
    ...
    if options.hash_files && is_file && st_size <= options.max_hash_bytes {
        digest = hash_file(&path, options.max_hash_bytes);
    }
...
fn entries_thread(
    ...
    if !dir_entry.file_type.is_dir() {
        let _ = tx.send(create_entry(&dir_entry, &options));
        return;
    }
        ...
        .process_read_dir(move |_, root_dir, _, children| {
            ...
            children.iter_mut().for_each(|dir_entry_result| {
                if let Ok(dir_entry) = dir_entry_result {
                    if tx.send(create_entry(dir_entry, &options)).is_err() {
                        return;
                    }
                }
            });
...
impl Scandir {
    ...
    /// Hash file contents (SHA-256). Off by default — metadata-only scans are much faster.
    pub fn hash_files(mut self, hash_files: bool) -> Self {
        self.options.hash_files = hash_files;
        self
    }

    /// Skip hashing files larger than this when hashing is enabled.
    pub fn max_hash_bytes(mut self, max_hash_bytes: u64) -> Self {
        self.options.max_hash_bytes = max_hash_bytes;
        self
    }
